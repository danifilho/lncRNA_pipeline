#!Rscript
# Vendored verbatim from Plant-LncRNA-pipeline-v2/Script/prediction_insersection.sh.
# ONLY change vs upstream: the cosmetic venn.diagram() call gets filename=NULL +
# grid.draw() so it renders under a newer VennDiagram. The lncRNA intersection
# logic (the actual result) is unchanged.
...
venn.plot <- venn.diagram(list(PlantLncBoost=PlantLncBoost, CPAT_plant=CPAT_plant, LncFinder_plant=LncFinder_plant, uniprot=uniprot),
  ...
  alpha = 1,
  fill = color,
  filename = NULL)   # compat: newer VennDiagram requires filename; NULL -> draw to device
grid.draw(venn.plot)
dev.off()  
